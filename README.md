...
```bash
python setup.py                  # installs everything, creates env files
supabase db reset                # apply supabase/migrations/ (needs Docker + supabase CLI)
python dev.py                    # starts DB + backend + frontend in one terminal
```

Then open http://localhost:3000 and sign up (or use a demo account — see RUN.md).
See [RUN.md](RUN.md) for the full setup, demo accounts, and a manual
